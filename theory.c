 ...
 - C 의 특징
    ...
    * 함수란 특정한 작업(기능)을 수행하도록 설계된 독립적인 프로그램
      ...
      12. 파일포인터를 이용한 파일 입출력
        ...
        13. 메모리 동적할당
            - 메모리 정적 할당: 기억공간의 데이터 영역과 스택 영역 이용
                    프로그램을 작성하는 단계에서 필요한 기억공간의 크기를 결정
            - 메모리 동적 할당: 기억공간의 힙 영역 이용
                    프로그램 실행 중에 입력되는 자료에 맞게끔 기억공간의 크기를 결정

            - 기억공간의 할당 영역 (데이터 영역, 힙 영역, 스택 영역)
              -> 데이터 영역: 전역변수와 static 변수가 저장되는 영역 => 프로그램 시작 시에 할당되고, 종료 시에 소멸한다.
              -> 스택 영역: 지역변수와 매개변수가 저장되는 영역 => 함수호출이 완료되면 사라진다.
              -> 힙 영역: 프로그래머의 필요에 의해 할당과 소멸이 이루어지는 영역 => 자유 기억공간이라 불리며, 실행이 되면서 크기가 늘고 줄어든다.

            - 메모리 정적 할당: 변수 선언과 배열 선언과 같이 프로그램을 작성하는 단계에서 필요한 기억 공간의 크기를 결정
                -> 변수 선언과 같이 할당시켜 줘야 할 기억공간의 한계 크기를 알고 작성해야 함. 따라서 잘 알고 있다면 쉽게 기억공간을 사용할 수 있고
                에러의 발생 확률을 줄인다.
                -> 하지만 사용할 기억공간의 크기를 정확히 알지 못한다면 기억공간의 낭비를 가져오게 된다.

            - 메모리 동적 할당: 힙 영역을 이용하여 프로그램 실행 중에 입력되는 크기에 맞게 기억공간을 확보
                -> 많은 자료를 처리하는 배열의 크기를 실행시간에 정의해야 하는 경우에 특히 유용하다.
                -> 프로그램 실행 시 기억 공간의 크기를 지정할 수 있고, 재조정도 가능하다.
                -> 시간이 지체되는 단점이 있다.

            - 메모리 동적 할당의 순서
                1. 기억공간을 동적으로 할당 받을 변수를 포인터를 이용하여 선언
                2. malloc() 함수 등을 이용하여 기억공간을 동적으로 할당 cf) calloc() 함수도 할당하지만, 0으로 초기화된다는 다른점이 있다.
                3. 기억공간의 사용이 끝나면 free() 함수를 이용하여 기억공간을 해제 => 명시적인 반납이 필요함! void free(void *p) 형식

            - 기억공간 관리 함수
              memcmp(): 기억공간에 들어있는 자료를 주어지는 크기만큼 비교하여, 같은지 여부를 알수 있게 해주는 함수
              memcpy(): 기억공간의 자료를 다른 기억 공간 영역으로 복사하기 위한 함수
              memset(): 기억공간의 자료를 지정한 문자로 채우는 함수, 할당된 기억 공간의 초기화나 내용 삭제를 위해 주로 사용
